...
public class UserFolderDAOTest {
    ...
    public void testUnorderedQuery() {
        ...
        assertEquals(SPARQLQueryUtil.getCrInferenceDefinitionStr() + "SELECT ?parent ?subject bif:either( bif:isnull(?lbl) , "
                ...
                + "> .?parent ?hasPredicate ?subject .  FILTER (?hasPredicate IN (<" + Predicates.CR_HAS_FILE + ">, <"
                + Predicates.CR_HAS_FOLDER + ">)) .  FILTER(?parent= ?parentFolder)  "
                + "{ SELECT ?subject (count(?file) AS ?fileCount)  WHERE { ?subject <" + Predicates.CR_HAS_FILE
                + "> ?file    } GROUP BY ?subject}  { SELECT ?subject (count(?folder) AS ?folderCount)  WHERE " + "{ ?subject <"
                + Predicates.CR_HAS_FOLDER + "> ?folder     } GROUP BY ?subject}OPTIONAL "
